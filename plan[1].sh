...
pkg_deps=(core/tomcat8 core/docker core/curl)
pkg_build_deps=(core/git core/maven core/postgresql)
...
do_install()
{
    ...
    # in the package.
    local source_dir="${HAB_CACHE_SRC_PATH}/${pkg_dirname}/${pkg_filename}/voting-app-web"
    local webapps_dir="$(hab pkg path core/tomcat8)/tc/webapps"
    cp ${source_dir}/target/votingapplication.war ${webapps_dir}/
    cp ${source_dir}/target/votingapplication.war /root/
